function PressHouse() {
    ...
    class ShortReports extends Article {
        ...
        toString() {
            let resultMessage = super.toString();
            resultMessage += `Original Research: ${this.originalResearch.title} by ${this.originalResearch.author}`
    ...
    class BookReview extends Article {
        ...
        addClient(clientName, orderDescription) {
            if (this.clients.some(x => x.clientname === clientName)) {
        ...
        toString() {
            let resultMessage = super.toString();
            resultMessage += `Book: ${this.book.name}`;
            if (this.clients.length != 0) {
                resultMessage += '\nOrders:\n';
                this.clients.forEach(element => {
                    resultMessage += `${element.name} - ${element.author}`;
                });
            }
            return resultMessage;
        }
